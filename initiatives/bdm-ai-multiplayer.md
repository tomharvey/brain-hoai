...
- **2026-06-23: Sophie Dodds' personal brief has evolved beyond the BDM Brain.** Her personal co-work folder structure is more developed than the BDM Brain itself — daily brief running in Notion, meeting summaries auto-Slacked. She's building in isolation without a shared layer. The BDM Brain needs to offer her something her individual setup can't: cross-BDM signals, ghost account detection. Her setup is the Stage 3 reference model; the shared brain is Stage 4. ([[2026-06-23-sophie-dodds-ai]])
- **2026-06-24: Matt Lees enterprise prospecting pipeline.** Apollo API giving 429 errors — hitting rate limits on the bulk prospecting workflow. Moving 600-company pipeline data into the shared brain. Also: Claude Design for high-stakes client decks. Apollo error is a short-term friction; not a fundamental blocker. ([[2026-06-24-matt-lees-ai-discovery]])
- 2026-06-15: BDM Dashboards — Adam + Ollie + Tom. **Architecture clarification:** Ollie's Phase 1 implementation uses a **Google Drive file sync** approach — markdown file structure synced via Google Drive desktop client, accessed through co:work. This is the architecture he has running with his engineering team. Differs from the Notion-based Phase 1 design in `cowork-implementation.md` — both are valid; the Google Drive approach is simpler to bootstrap. **Wednesday Jun 18 workshop confirmed.** Goal: BDM team installs the brain, experiments with building skills, co-creates dashboard template and metric definitions. Setup prereqs: Tom tests file structure + gets Google Drive on team computers (AI-111); Ollie writes instructions (AI-112). Adam's three-day vision confirmed: 2 days out with brokers + 1 day on calls/pipeline + remaining time using the brain for strategy/planning. → [[2026-06-15-bdm-dashboards]]
