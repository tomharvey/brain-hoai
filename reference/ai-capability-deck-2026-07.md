...
- Denominator caveat: `ai-activation-map.md` (updated Jun 25) uses ~63 in scope; this deck uses the June assessment scope plus new joiner Kayleigh (57). Reconcile before Ed forwards numbers to Admiral Pioneer.
...
- At Stage 1: Anneliese (install blocked — AI-149) · Queency (daily use, single-shot) · Kayleigh (new in claims, heavy personal use, first work 1:1 booked)
...
- Operations context (regrouped Jul 2: claims folded in): Adam Sandle (2, fortnightly coaching AI-131) and Kayleigh (1 — brand-new, heavy personal AI use, none at work yet) join; Antton (CCO) remains the unassessed coverage gap, so n=7 understates the team by one.
...
| Ops | Jonny · Fred · Adam Sandle · Kayleigh | Stages 1–2; guided-only wins; Sandle on fortnightly coaching (AI-131); Kayleigh new — heavy personal use, none at work yet |

**Presenter notes**
- At-risk framing: nobody on this list is refusing. Four groups, four different interventions: *blocked* (Anneliese — a Python install; Queency — a filtering bug), *unmeasured* (Ben, Daisy — assess before judging), *pulled back* (Stephen, Aleks — the craft conversation, not pressure), *habit not formed* (Michael, Milan, Rakhee, Phoebe, Adam Sandle — coached reps on real work), plus *new starters* (Kayleigh, Connie returning — onboarding, not remediation). A single "level up or else" message would misfire on three of the four groups.
...
**Result: 25 of 50 verified builders (50%) · 10 assisted/decayed (20%) · 15 no build evidence (30%).** (Kayleigh added Jul 2, no artifact yet.)
