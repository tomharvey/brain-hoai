...
Session type slugs: `ad-hoc`, `granola`, `ghost-check`

**Scope: interactive sessions only.**
Scheduled tasks (morning brief, pre-meeting brief, EOD nudge, weekly pulse) do NOT
write session logs. They use server-side MCP connectors only (Slack, HubSpot, Granola,
Notion, GCal, Gmail) and cannot access the local filesystem.

Session logs are generated by `/session-close` at the end of interactive conversations.
Because Drive syncs to all machines, `/brain-health deep` (run manually by Adam) can
read logs from all BDMs (`logs/**/*.md`) without extra configuration.

---

## Session close — always run this

At the end of every interactive conversation, before closing, run `/session-close`.

This writes a structured log entry to the observability log. The `/brain-health deep`
command (run manually by Adam) reads these entries, finds patterns, and generates
ranked improvements with specific rewrites. The brain gets better from real usage —
but only if sessions are logged.

Do NOT run session-close for scheduled tasks (morning brief, pre-meeting brief, EOD nudge,
weekly pulse). Scheduled tasks are server-side and cannot write to the local filesystem.
